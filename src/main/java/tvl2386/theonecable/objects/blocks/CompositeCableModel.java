 ...
 */
public class CompositeCableModel implements IBakedModel {

    public CompositeCableModel(IBakedModel i_modelCore, IBakedModel i_modelUp, IBakedModel i_modelDown,
    ...
    public List<BakedQuad> getQuads(@Nullable IBlockState blockState, @Nullable EnumFacing side, long rand) {
        ...
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_UP)) {
            quadsList.addAll(modelUp.getQuads(extendedBlockState, side, rand));
        }
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_DOWN)) {
            quadsList.addAll(modelDown.getQuads(extendedBlockState, side, rand));
        }
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_WEST)) {
            quadsList.addAll(modelWest.getQuads(extendedBlockState, side, rand));
        }
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_EAST)) {
            quadsList.addAll(modelEast.getQuads(extendedBlockState, side, rand));
        }
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_NORTH)) {
            quadsList.addAll(modelNorth.getQuads(extendedBlockState, side, rand));
        }
        if (isLinkPresent(extendedBlockState, BlockCable.LINK_SOUTH)) {
    ...
    public TextureAtlasSprite getParticleTexture() {
        TextureAtlasSprite textureAtlasSprite = Minecraft.getMinecraft().getTextureMapBlocks()
                .getAtlasSprite("theonecable:blocks/mbe05_block_3d_web");
